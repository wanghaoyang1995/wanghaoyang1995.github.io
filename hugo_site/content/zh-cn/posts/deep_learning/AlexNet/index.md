...
lastmod = "2025-11-12T10:01:21+08:00"
draft = false
math = true
...
在图像数据上减少过拟合最简单且常见的方法就是使用标签保留变换（Label-preserving Transformations）来扩展数据集。我们使用了两种不同的数据增广方式，二者均允许以很小的计算量完成对原始图像的转换，因此转换后的图像不需要被存储到磁盘上。  
在我们的实现中，由执行在CPU上的Python代码来产生转换后的图像，而GPU用于在前一个batch图像上进行训练。  
所以这些数据增广方案，在效果上是无计算负担的。

第一种数据增广方式由图像生成变换和水平翻转组成。  
我们从原始\(256 \times 256\)的图像中提取随机的\(224 \times 224\)图块（以及它们的水平镜像），利用这些提取的图片训练我们的网络。  
这增加了我们的训练集的大小到原来的2048倍，尽管最终的训练样本是高度相关的。  
如果没有这个方案，我们的网络会产生严重的过拟合，这将强制我们使用更小的模型。  
在测试阶段，该模型对5个\(224 \times 224\)图块（四个角落位置、一个中心位置）以及它们的水平镜像（因此一共10个图块）进行预测，并且将模型softmax层产生的10个图块的输出作平均。

第二种数据增广方式是改变训练集图像的RGB通道强度。  
具体来说，我们对整个ImageNet训练集的RGB像素值作了主成分分析（PCA）。  
对于每一个训练图片，我们将其加上主成分的若干倍，这些倍数的大小正比于对应的特征值乘以一个从高斯分布（均值0，方差0.1）中采样的随机值。  
因此，每一个RGB图像的像素 \(I_{xy}^{R},I_{xy}^{G},I_{xy}^{B}\) 被加上如下的量：

$$ [\textbf{p}_1, \textbf{p}_2, \textbf{p}_3][\alpha_1 \lambda_1, \alpha_2 \lambda_2, \alpha_3 \lambda_3]^T $$

其中 \(\textbf{p}_i\) 和 \(\lambda_i\) 分别是RGB像素值的 \(3 \times 3\) 协方差矩阵的第 i 个特征向量和特征值，\(\alpha_i\) 是前面提到的随机值。  
每个 \(\alpha_i\) 在一张特定图像的所有像素上只被生成一次，直到下次该图像再次用于训练时重新生成。  
这个方案近似捕捉了自然图片的一个重要特征，即对象实体在不同光照强度、颜色的变化下是恒定不变的。  
这个方案将 top-1 错误率降低了超过1%。
